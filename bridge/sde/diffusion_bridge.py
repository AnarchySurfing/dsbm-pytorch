...
# 基于方差爆炸（VE）的薛定谔桥扩散模型
class DBDSB_VE:
    def __init__(self, sig, num_steps, timesteps, shape_x, shape_y, first_coupling, mean_match=False, ot_sampler=None,
                 eps=1e-4, **kwargs):
    ...
    def record_langevin_seq(self, net, samples_x, init_samples_y, fb, sample=False, num_steps=None, **kwargs):
        ...
        if num_steps is None:
            ...
            timesteps = np.interp(np.arange(1, num_steps + 1) / num_steps,
                                  np.arange(self.num_steps + 1) / self.num_steps,
                                  [0, *np.cumsum(timesteps.cpu())])
    ...
    def generate_new_dataset(self, x0, y0, x1, sample_fn, sample_direction, sample=False, num_steps=None):
        ...
        if sample_direction == 'f':
            ...
            zstart = x1
        # 通过朗之万动力学从起点采样到终点
        zend = self.record_langevin_seq(sample_fn, zstart, y0, sample_direction, sample=sample, num_steps=num_steps)[0][
               :,
               -1]
        if sample_direction == 'f':
            z0, z1 = zstart, zend
...
class DBDSB_VP(DBDSB_VE):
    def __init__(self, sig, num_steps, timesteps, shape_x, shape_y, first_coupling, mean_match=False, ot_sampler=None,
        ...
        super().__init__(sig, num_steps, timesteps, shape_x, shape_y, first_coupling, mean_match=mean_match,
                         ot_sampler=ot_sampler, eps=eps, **kwargs)
