...
def DBDSB_CacheLoader(sample_direction, sample_fn, init_dl, final_dl, num_batches, langevin, ipf, n, refresh_idx=0, refresh_tot=1, device='cpu'):
    ...
    if ipf.accelerator.is_main_process:
        ...
        num_plots_grid = 100
        ipf.plotter.save_image(torch.from_numpy(fp[:num_plots_grid, 0].copy()), f'cache_{sample_direction}_{n:03}_x0', ipf.output_dir, domain=0)
        ipf.plotter.save_image(torch.from_numpy(fp[:num_plots_grid, 1].copy()), f'cache_{sample_direction}_{n:03}_x1', ipf.output_dir, domain=1)
